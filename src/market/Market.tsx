...
export function MarketSave({topic,compact=false}: {topic:Topic;compact?:boolean}) {
  const {state,update,requireLogin}=useForum();
  const saved=state.saves.includes(topic.id);
  return <button className={`market-save ${compact?'compact':''}`} aria-label={`${saved?'取消收藏':'收藏'}${compact?topic.title:''}`} aria-pressed={saved} onClick={()=>{if(requireLogin())update(s=>({...s,saves:s.saves.includes(topic.id)?s.saves.filter(id=>id!==topic.id):[...s.saves,topic.id]}))}}><Heart size={15}/>{!compact&&<span>{saved?'已收藏':'收藏'}</span>}</button>;
}
export function MarketHome() {
  ...
  return <div className="market-page">
    ...
    <div className="market-grid">{topics.map(topic=>{const author=state.users.find(u=>u.id===topic.authorId)!;return <article className="market-card" key={topic.id} data-product={topic.id}><TopicLink className="market-product-link" to={'/topic/'+topic.id}><div className="market-photo"><MarketImage topic={topic}/></div><h2>{topic.title}</h2><div className="market-price-row"><strong>{priceLabel(topic)}</strong><span>{(topic.market?.baseSaves??0)+Number(state.saves.includes(topic.id))} 已收藏</span></div></TopicLink><div className="market-byline"><Avatar name={author.name} color={author.color} avatarId={author.avatarId}/><span>{author.name}</span><MarketSave topic={topic} compact/></div></article>})}</div>
