...
export function MarketHome() {
  ...
  return <div className="market-page">
    ...
    <div className={mode==='wanted'?'market-wanted-list':'market-grid'}>{topics.map(topic=>{const author=state.users.find(u=>u.id===topic.authorId)!;return <article className="market-card" key={topic.id} data-product={topic.id}><TopicLink className="market-product-link" to={'/topic/'+topic.id}>{mode!=='wanted'&&<div className="market-photo"><MarketImage topic={topic}/></div>}<h2>{topic.title}</h2><div className="market-price-row"><strong>{priceLabel(topic)}</strong>{mode!=='wanted'&&<span>{(topic.market?.baseSaves??0)+Number(state.saves.includes(topic.id))} 已收藏</span>}</div></TopicLink><div className="market-byline">{mode!=='wanted'&&<Avatar name={author.name} color={author.color} avatarId={author.avatarId}/>}<span>{author.name}</span><MarketSave topic={topic} compact/></div></article>})}</div>
