...
test('shared icon actions persist in forum and market details and seller controls stay aligned', async ({ page }) => {
  ...
  for (const id of ['topic-2','market-demo-6']) {
    ...
    if(id === 'topic-2') await expect(like).toHaveText('');
    else await expect(like).toHaveCount(0);
    await expect(save).toHaveText('');
    if(id === 'topic-2') await like.click();
    await save.click();
    if(id === 'topic-2') await expect(like).toHaveAttribute('aria-pressed','true');
    await expect(save).toHaveAttribute('aria-pressed','true');
    await expect(save).toHaveCSS('border-width','0px');
    await expect(save).toHaveCSS('background-color','rgba(0, 0, 0, 0)');
    const fill=await save.locator('svg').evaluate(e=>getComputedStyle(e).fill);
  ...
  await expect(page.locator('.original-post .reaction-save')).toHaveAttribute('aria-pressed','true');
});
